...
func (p *PGStorage) RecordFillInfo(para *HTLCHashInfo,cpppara *HTLCHashInfo, orderid string, intentid string) (error) {
    ...
    fillconfirminfo.OrderId  = orderid
    fillconfirminfo.IntentId  = intentid
    ...
    err = s.QueryRow(orderid).Scan(
 	...
 	fillconfirminfo.UserPubKey = fillconfirminfo2.CpPubKey
	fillconfirminfo.CpPubKey   = fillconfirminfo2.UserPubKey
    ...
    s, err = p.db.Prepare("SELECT chainid FROM assets WHERE assetId =$1")
    if err != nil {
    	fmt.Printf("\nffffffffffffffff",err)
		return err
	}

	err = s.QueryRow(fillconfirminfo.AssetId).Scan(
		&fillconfirminfo.MakerChain,			
	)

	err = WithTransaction(p.db, func(tx *sql.Tx) error {
		_, err := tx.Exec(
			`INSERT INTO fills (userPubKey, cpPubKey, orderId, intentId,makerchain,takerchain, HTLCHash, assetid, chainid, memo, createdAt, signature, settled, preimage, timeout, quantity, cppHTLCHash, cppassetid, cppchainid,cppmemo, cppcreatedat,cppsignature,cpppreimage,cpptimeout,cppquantity) 
...
func (p *PGStorage) RecordLocalFillInfo(para *HTLCHashInfo, orderid string, pubkey string) (error) {
	fillconfirminfo := &domain.FillConfirmInfo{} 
    fillconfirminfo.Settled = true
    fillconfirminfo.HTLCHash = para.HTLCHash
    fillconfirminfo.PHash    = para.PHash
    ...
    s, err:= p.db.Prepare("SELECT orderid, createdAt,  memo, makerasset, signature, quantity, userPubkey FROM orders WHERE orderid =$1")
    if err != nil {
    	fmt.Printf("\n1111111111111",err)
		return err
	} 
	...
	fillconfirminfo.UserPubKey = pubkey
	...
	err = WithTransaction(p.db, func(tx *sql.Tx) error {
		_, err := tx.Exec(
			`INSERT INTO fills (userPubKey, cpPubKey, orderId, makerchain, HTLCHash, assetid, chainid, memo, createdAt, signature, settled, preimage, timeout, quantity) 
...
func (p *PGStorage) RecordCppFillInfo(para *HTLCHashInfo, intentid string) (error) {
	fillconfirminfo := &domain.FillConfirmInfo{} 
    fillconfirminfo.Settled = true
    fillconfirminfo.HTLCHash = para.HTLCHash
    fillconfirminfo.PHash    = para.PHash
    fillconfirminfo.Timeout  = para.Timeout
    s, err := p.db.Prepare("SELECT intentId, orderId, createdAt,  memo, takerasset, signature, quantity FROM intent WHERE intentid =$1")
    if err != nil {
    	fmt.Printf("\n4444444444444",err)
		return err
	} 
	...
	err = s.QueryRow(fillconfirminfo.AssetId).Scan(
		&fillconfirminfo.TakerChain,			
	)
	err = WithTransaction(p.db, func(tx *sql.Tx) error {
		_, err := tx.Exec(
			`UPDATE fills set intentId=$1,takerchain=$2, cppHTLCHash=$3, cppassetid=$4, cppchainid=$5,cppmemo=$6, cppcreatedat=$7,cppsignature=$8,cpppreimage=$9,cpptimeout=$10,cppquantity=$11 WHERE orderId  = $12`,
			fillconfirminfo.IntentId,
			fillconfirminfo.TakerChain,
			fillconfirminfo.HTLCHash,
			fillconfirminfo.AssetId,
...
func (p *PGStorage) MakerRecordLocalFillInfo(para *HTLCHashInfo, intentId string) (error) {
	fillconfirminfo := &domain.FillConfirmInfo{} 
    fillconfirminfo.Settled = true
    fillconfirminfo.HTLCHash = para.HTLCHash
    fillconfirminfo.PHash    = para.PHash
    ...
    s, err:= p.db.Prepare("SELECT orderid, intentid,createdAt,  memo,makerasset, takerasset,signature, quantity, userPubkey,cpPubkey FROM intent WHERE intentid =$1")
    if err != nil {
		return err
	} 
    ...
    fillconfirminfo.AssetId = fillconfirminfo.MakerChain
	err = WithTransaction(p.db, func(tx *sql.Tx) error {
		_, err := tx.Exec(
			`INSERT INTO fills (userPubKey, cpPubKey, orderId,intentid, makerchain, takerchain,HTLCHash, assetid, chainid, memo, createdAt, signature, settled, preimage, timeout, quantity) 
